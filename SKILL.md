...
- 各子 job 独立走完评估 + finalize 后，**一键聚合**跨 sheet 交付物到父目录：
  ...
  ```
  自动发现父目录下含 state.json 的子 job（`--sheets` 可指定顺序/子集）→ 产出 `<文件名>_corrected.xlsx`（镜像原始多 sheet 结构，仅替换译文列）+ `<文件名>_LQE报告.xlsx`（汇总 sheet：各子表分数 + 按词数加权总分 `(1-ΣL/Σwordcount)×100`，再附各子表 LQE Results 明细）。段→行映射用 `segment.id == src 数据行枚举位置`，与 `lqe_io.read/export` 一致。
- 跨 sheet 的合并交付物（汇总报告、还原原结构的修正文件）放父目录 `jobs/<文件名>/`，**不得散落在 `jobs/` 根**
- 还原的修正文件须镜像原始多 sheet 结构（含空行/全部列），仅替换译文列。**⚠ read_only 空行坑**：openpyxl 普通模式 `load_workbook` 静默裁掉全空尾行（实测社媒 src 真有 171 行 / XML `dimension="A1:B171"`，普通模式 `max_row` 只报 88 → 丢 83 行还不报错）；凡「镜像原始行结构」必须 `load_workbook(..., read_only=True)` 读。`aggregate_sheets.py` 已钉死 read_only；**`lqe_io.cmd_export` 仍用普通模式**，单 sheet 输入含尾部空行时同样丢尾部空行——**已评估暂不修（2026-06-22 定）**：影响窄，仅「Mode B 单 sheet + 源带尾部空行 + 下游卡行数对齐」才暴露，且不丢译文/不错位；多 sheet 及任何需纯净逐行还原的交付一律走 `aggregate_sheets.py`。
