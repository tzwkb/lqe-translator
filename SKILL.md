...
**分批方式二选一**：`lqe_chunk.py split` 按**段数**（~200/块）切，适合多数；`lqe_batch.py plan/merge` 按**输出 token 体量**自适应分批 + 断点续跑（写 `manifest.json`），适合每段产出大（如泰文剧情/对话整段完整修正、易超 64K）的场景——批级失败只重跑该批、merge 幂等缺批占位。
...
2. `lqe_chunk.py split`：① 按 (源,译) **去重**——完全相同句段只评一次（写 `dedup_map.json`），省冗余 + 杜绝同句异判；② term_hits **最长匹配覆盖过滤**——被同位置更长术语覆盖的子词不喂（如 `绒光优优` 不带 `优优`）；③ 每段标 `kind`（name/desc）供 lens 门控。每块 ~200 段 → `chunks/chunk_NN.json`。**大/密集 job 防单 agent 过载/崩溃**：加 `--char-budget N`（按源+译字符量切块，密集段自动少装、各块负载均匀；`--size` 仍作段数硬上限）——实证固定 200 段时块负载 14K–20K 字严重不均、最密块（如术语机制密集段）撑爆 A agent；`--char-budget` 把每块压到 ~N 字、配「断点优先」即细粒度断点（崩/中断只丢一小块）。**默认不传=固定 200，与旧版逐字节一致、不改结果**；启用唯一副作用=块内一致性窗口随块变小（建议每块 ~80–120 段，别设太小）
3. **每块 × 每个适用 lens 派 1 subagent**，读 `docs/lenses/_common.md` + 自己的 lens 文件（`T/A/G/R.md`）+ 各自 background/sg/lang_notes/adjudications + chunk，写 `chunk_NN.<L>.json`（如 `chunk_03.A.json`）。lens 分工见上表（T/A 跑全部段，G/R 跑 desc 段）。**每个 lens 先 pilot 1 块验证格式再 fan-out**；T 是基准轴必须先齐。**派发 prompt 必含输出纪律（防 64K 崩）：评估结果只用 Write 写进 `chunk_NN.<L>.json`、最终聊天回复 ≤3 行纯文本统计、绝不把 JSON/修正译文/逐条明细写进回复**——实证某 A agent 把整块结果怼进聊天响应触发 64K output 上限而崩、整块进度丢失。
   ...
   - **存活只认落盘 + 绝不杀活 agent（铁律）**：判断 agent 停没停**只认该 agent ID 的 完成/失败/kill 通知**——**没收到通知=还在跑**；运行时长、空磁盘、`.output` transcript 大小（127 字节缓冲假象）**一律不是死亡证据**（atomic 写下"慢但活"与"死"表象完全相同，物理无法区分，"没落盘+跑得久"推不出"死了"）。**慢就等**（任务无 deadline，可 Monitor 有界等待轮询产出文件）；**想更快/对冲真卡死就另派替补并行、绝不 TaskStop 原 agent**（只认文件后写覆盖、零进度损失）。**TaskStop 仅在 通知确认失败 或 用户明确放弃 时用**——曾据"92 分钟无产出超先例"误判挂死而 kill 掉其实在 reading 的 agent、毁 ~90 分钟进度（缺席证据≠死亡证据，见 [[feedback_no_kill_live_agent]]）。
