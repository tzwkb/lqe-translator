...
def terminology_issue_fields(issue: object) -> dict | None:
    ...
    if (
        ...
        and isinstance(spans, dict)
        and set(spans) == {"source", "target"}
        and isinstance(spans.get("source"), list)
        and bool(spans["source"])
        and isinstance(spans.get("target"), list)
    ):
        return {
            "term_source": source,
            "expected_targets": list(targets),
            "term_spans": copy.deepcopy(spans),
        }
    return None
