...
meta = [
    ...
    "状态：PM 已批准（2026-06-12）并全部实施——N5–N9、#3 通用标签对账+custom count_match、#7 缩写大小写（Major）、#10 省略号混用检查；#2 泰语数词已纳入；#5 的 20 字分档随 N2 实施线记录",
    "项目负责人反馈已全部实施：#2 仅带量词触发并纳入泰语；#3 增加通用标签模式检查（无需再提供样例，特殊格式后续按项目配置）；#5 阈值 20 字；#7 仅检查全大写缩写并按严重问题处理；#10 检查文件内省略号样式是否一致",
    "输出约定：errors 非空且 corrected=null 是合法结果；报告统一使用“原译、建议译文、处理方式”，建议译文工作簿保留源 workbook 结构且只写非空 corrected，不新增状态列或工作表",
]
r = 3
for m in meta:
...
HEADERS = ["序号", "项目负责人建议", "是否需要修改", "修改程度", "如何修改（实现方案与误报防控）", "涉及文件", "需要项目负责人提供或确认"]
...
ROWS = [
    ...
    ["2", "中文数字对应（如「三次」→ 3/three）",
     ...
     "请确认两件事：① 数字『一』要不要也查——查的话误报会变多，建议只查『一次/一名』这类带量词的；② 泰语版要不要这期一起做"],
    ...
    ["5", "异源同译（不同源文译成相同译文）",
     ...
     "请确认：多长的句子译文完全相同才算可疑——建议 20 字以上才报，『确定/确认』这类短按钮词不报"],
    ["6", "单词连续重复（如 the the）",
     "需要",
     "新增内置检查（拟编 N7）",
    ...
    ["7", "特定术语大小写（HP/ATK/人名等必须大写）",
     ...
     "请确认两件事：① 大小写写错算多重——建议一般情况算轻微错误，HP/ATK 这类全大写缩写写错算严重错误；② 要不要在术语表加一列，标出哪些词必须严格按表里的大小写"],
    ["8", "同个词内大小写混乱（如 AppLe）",
     "需要",
     "新增内置检查（拟编 N8）",
