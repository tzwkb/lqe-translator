...
HEADERS = ["序号", "PM 建议", "是否需要修改", "修改程度", "如何修改（实现方案与误报防控）", "涉及文件", "待确认"]
...
ROWS = [
    ["1", "译文句号缺失（建议用 AI 识别源译句号数量是否对应）",
     ...
     "泰语等不用句号的语言在语言层默认关闭本项（languages/th.json）。",
     "scripts/lqe_io.py；languages/th.json（默认关）；清单文档",
     "无"],
    ["2", "中文数字对应（如「三次」→ 3/three）",
     ...
     "「一」的触发范围（建议仅量词强模式）；泰语数词映射是否本期纳入"],
    ["3", "标签（TAG）正则按项目单独配置（现内置仅燕云 #G…#E）",
     "需要（方向正确，架构已支持一半）",
     "扩展项目自定义检查架构（约 15 行）＋各项目配置",
     "现行 projects/<名>/checks.json 的 custom 正则已实现「按项目喂正则」，但仅支持单边命中报错，无法做源↔译对账。"
    ...
    ["6", "单词连续重复（如 the the）",
     ...
     "仅空格分词译文生效（泰语跳过——泰文表重复另有 ๆ 标记体系）；Grammar Minor；对话体重复由 AI 甄别。",
     "scripts/lqe_io.py；清单文档",
     "无"],
    ["7", "特定术语大小写（HP/ATK/人名等必须大写）",
     ...
     "归类与严重度分档需 PM 拍板；是否加 case_sensitive 列"],
    ["8", "同个词内大小写混乱（如 AppLe）",
     "需要",
     "新增内置检查（拟编 N8）",
