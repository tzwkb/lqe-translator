...
公共协议见 `common.md`。本模块只处理 Audience appropriateness、Culture specific reference、Unidiomatic。必须覆盖全部分配 id；无问题时写空 `"issues"`。每条问题使用 `"needs_confirmation"` 和 `"edit"`，不得输出 corrected。
